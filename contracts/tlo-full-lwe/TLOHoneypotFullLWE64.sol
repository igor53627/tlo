...
/// @title TLOHoneypotFullLWE64 - n=64 LWE dimension (~128-bit security)
contract TLOHoneypotFullLWE64 is IHoneypot {
    ...
    // n=64: CT_SIZE = 64*2 + 2 = 130 bytes, GATE_SIZE = 3 + 4*130 = 523 bytes
    ...
    function scheme() external pure override returns (string memory) { return "tlo-full-lwe-64"; }
    function encryptedGates() external pure override returns (uint256) { return 640; }
    function estimatedGas() external pure override returns (uint256) { return 3_000_000; }
    ...
    function _evaluate(bytes32 input) internal view returns (bool) {
        ...
        // Derive 4 x 256-bit secrets for n=64 (64 x u16 = 1024 bits)
        ...
        assembly {
            ...
            for { } lt(dataPtr, endPtr) { dataPtr := add(dataPtr, 523) } {
                ...
                // CT offset: 3 + ttIdx * 130
                let ctPtr := add(dataPtr, add(3, mul(ttIdx, 130)))
                
                // Read a vector: 128 bytes = 4 mloads
                let a0 := mload(ctPtr)
