...
\begin{abstract}
We present \tlo{} (Topology-Lattice Obfuscation), a practical circuit obfuscation framework for smart contracts. Security derives from a two-layer defense: a topology layer using structural mixing defeats structural and statistical attacks (empirically validated), while the \lwe{} layer computes inner products on-chain to hide control functions. Security is based on uniform-secret \lwe{} hardness (${\sim}49$-bit classical / ${\sim}45$-bit quantum estimate with $n{=}64$; see \S\ref{sec:security-estimates}) combined with topology properties, providing post-quantum resistance (assuming \lwe{} quantum-resistance).
...
\item \textbf{Layer 2 (\lwe{}):} On-chain inner products defeat semantic attacks. \emph{Security: computational (${\sim}49$-bit with uniform secrets).}
\end{enumerate}
...
\item \textbf{Post-Quantum Security:} ${\sim}49$-bit classical / ${\sim}45$-bit quantum estimate via uniform-secret \lwe{} ($n{=}64$); ${\sim}81$-bit for $n{=}128$; see \S\ref{sec:security-estimates}.
\end{enumerate}
...
Using dual-attack analysis (which applies regardless of secret distribution), our parameters ($n{=}64$, $q{=}65521$, $\sigma{=}\sqrt{q}/4$) yield ${\sim}49$-bit security. This is suitable for \emph{eventually-expiring secrets} with hour-to-day lifetimes. For $n{=}128$: ${\sim}81$-bit; for $n{=}256$: ${\sim}132$-bit (NIST-level).
...
\bottomrule
\end{tabular}
\caption{Gas costs by \lwe{} dimension (uniform-secret security estimates).}
\end{table}
...
\tlo{} achieves 6/6 resistance against our attack matrix at ${\sim}2.58$M gas ($n{=}64$, ${\sim}49$-bit uniform-secret \lwe{} security). Post-quantum resistant (assuming \lwe{} quantum-resistance). Deployment requires only a standard smart contract with timestamp expiry.

\textbf{Key contributions:} On-chain \lwe{} inner products for true CF hiding; wrong-key-gives-garbage property combined with 1-bit oracle interface; discovery that uniform-secret \lwe{} provides stronger security than small-secret variants.
