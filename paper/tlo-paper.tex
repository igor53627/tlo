...
\begin{itemize}
  \item Computational security based on \lwe{} hardness (see Table~\ref{tab:lwe-security} for estimates: ${\sim}49$-bit classical for $n{=}64$ with uniform secrets) plus empirical heuristics (topology layer)
...
\begin{itemize}
  ...
  \item Error: Gaussian with $\sigma = \sqrt{q}/4$
\end{itemize}
...
$n{=}32$ & ${\sim}22$-bit & ${\sim}20$-bit & 1.27M & Low security \\
$n{=}64$ & ${\sim}49$-bit & ${\sim}45$-bit & 2.58M & Short-lived secrets \\
$n{=}128$ & ${\sim}81$-bit & ${\sim}74$-bit & ${\sim}5$M & Medium-lived secrets \\
$n{=}256$ & ${\sim}132$-bit & ${\sim}120$-bit & ${\sim}10$M & NIST-level security \\
\bottomrule
\end{tabular}
\caption{\lwe{} security estimates for uniform-secret variant ($q{=}65521$, $\sigma{=}\sqrt{q}/4$). Uniform secrets resist primal attacks; estimates based on dual attack. Gas costs are for 64-wire/640-gate circuits.}
\end{table}

\textbf{Important:} Our default parameters ($n{=}64$) provide ${\sim}49$-bit classical security. This is suitable for \emph{eventually-expiring secrets} with lifetimes of hours to days (e.g., MEV windows, auction rounds). For longer-lived secrets, use $n{\geq}128$ (${\sim}81$-bit) or $n{=}256$ for NIST-level security. We validated these estimates via direct lattice attacks using fpylll/BKZ; see \S\ref{sec:attack-validation}.
...
\textbf{$n{=}64$} & \textbf{${\sim}49$-bit} & \textbf{2.58M} & \textbf{8.6\%} \\
\bottomrule
\end{tabular}
...
For the paper, we use $n{=}64$ providing ${\sim}49$-bit classical security---suitable for eventually-expiring secrets (hours to days).
...
\begin{itemize}
  ...
  \item \textbf{$n{=}64$:} Attack running for 9+ hours on dedicated server (still in progress at time of writing). Extended runtime validates that ${\sim}49$-bit security is realistic.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{\lwe{} layer (computational):} On-chain inner product computation hides control functions, defeating semantic attacks. ${\sim}49$-bit classical security with $n{=}64$ using uniform secrets (see Table~\ref{tab:lwe-security}).
\end{itemize}
